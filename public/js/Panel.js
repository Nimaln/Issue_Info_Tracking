...
Autodesk.ADN.Viewing.Extension.UIComponent = function (viewer, options) {
  ...
  Autodesk.ADN.Viewing.Extension.UIComponent.Panel = function(
    ...
    var html = [
      '<div class="uicomponent-panel-container">',
        '<div class="uicomponent-panel-controls-container">',
          ...
          '<br>',
          '<div>',
            '<button class="btn btn-info" id="' + baseId + 'issBtn">',
              '<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Add Issue',
            '</button>',
            '<input class="uicomponent-panel-input" type="text" placeholder=" Name (default: Date/Time)" id="' + baseId + 'itemName1">',
          '</div>',
          '<br>',
          '<div>',
    ...
    $('#' + baseId + 'addBtn').click(function(){
        console.log('add button funcion entered')
    ...
    body: JSON.stringify({"_id":item.id,"val":name.toString(),"category":"Info"})
    }).then(res => res.json())
    ...
    $('#' + baseId + 'issBtn').click(function(){
        console.log('Issue button funcion entered')

      var name1 =  $('#' + baseId + 'itemName1').val();
      ...
      var item1 = {
        name1: name1,
        id1: newGUID(),
        handler: function (){alert('Item: ' + name1 + ' clicked!')}
      }
    ...
    body: JSON.stringify({"_id":item1.id1,"val":name1.toString(),"category":"Issue"})
    }).then(res => res.json())
    .then(res => console.log(res)); 
      ...
      var html = [
        ...
        '<div class="list-group-item uicomponent-panel-item" id="' + item1.id1 + '">',
          name1,
        '</div>'
